...
const { ensureLoggedIn, ensureAdmin, ensureCorrectUserOrAdmin, authenticateJWT } = require("../middleware/auth");
...
/** POST /user/:username/jobs/:id => { applied: jobId } 
 * allows user to apply for a job (or an admin to do it for them)
 * Authorization required: login, admin, or current user
*/
router.post("/:username/job/:job_id", ensureLoggedIn, ensureCorrectUserOrAdmin, async function (req, res, next) {
  try{
    const { job_id } = await User.apply_for_job(req.params.username, req.params.job_id);
